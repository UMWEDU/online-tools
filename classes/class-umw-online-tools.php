...
<aside class="' . implode( ' ', $c ) . '">
	...
	function admin_init() {
		...
		add_settings_section( 'umw-toolbar-settings-section', __( 'UMW Global Toolbar Settings' ), array(
			$this,
			'settings_section'
		), 'general' );
		...
		foreach ( $cbfields as $name => $field ) {
			...
			), 'general', 'umw-toolbar-settings-section', $args );
		}
	}
	...
	function settings_field_checkbox( $args = array() ) {
		...
		?>
        <p><input type="checkbox" name="umw-toolbar-settings[<?php echo $args['name'] ?>]"
                  id="<?php echo $args['id'] ?>" value="1"<?php checked( $vals[ $args['name'] ] ) ?>/>
